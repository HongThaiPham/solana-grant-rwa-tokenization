...
impl<'info> MintCarbonToken<'info> {
    pub fn handler(&mut self, amount: u64) -> Result<()> {
        ...
        let mint_with_extension = StateWithExtensions::<spl_token_2022::state::Mint>::unpack(&nft_minter_data)?;
        metadata = mint_with_extension.get_variable_len_extension::<TokenMetadata>()?;
    }
       

        let additional_metadata = metadata.additional_metadata.clone();
        let available_credits = additional_metadata.iter().find(|(key, _)| key == "available_credits");

        if let Some((_, value)) = available_credits {
            let available_credits = value.parse::<u64>().unwrap();
            require!(available_credits >= amount, MyErrorCode::InsufficientCredits);

            {
                ...
                mint_to(CpiContext::new_with_signer(self.token_program.to_account_info(), MintTo {
            ...
            // update available credits
                ...
                token_metadata_update_field(CpiContext::new_with_signer(self.token_program.to_account_info(), TokenMetadataUpdateField {
                ...
                let minted_credits = additional_metadata.iter().find(|(key, _)| key == "minted_credits");

                let mut updated_minted_credits = amount;
                if let Some((_, value)) = minted_credits {
                    let minted_credits = value.parse::<u64>().unwrap();
                    updated_minted_credits = minted_credits.checked_add(amount).unwrap();
                }
